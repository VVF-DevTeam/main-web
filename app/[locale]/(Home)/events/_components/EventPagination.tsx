...
const EventPagination = ({
  ...
  const createPageUrl = useCallback((page: number) => {
    ...
    params.set(pageType + 'Page', page.toString())
    return `?${params.toString()}`
  }, [searchParams, pageType, totalPages])
  ...
  }, [router, searchParams, pageType, totalPages, currentPage])
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      {showPageInfo && totalItems && (
        <div className="text-sm text-muted-foreground">
          Showing page {currentPage} of {totalPages} ({totalItems} total {pageType} events)
        </div>
      )}
