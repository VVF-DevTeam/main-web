...
const EventButton = ({ eventKeyName, eventType }: EventButtonProps) => {
  ...
  return (
    <Link href={`/events/${eventType.toLowerCase()}/${eventKeyName}`}>
      <Button
        size={'sm'}
        className="all ease text-pretty border-2 border-bgColor-brand bg-slate-100 text-sm font-bold text-bgColor-brand900 transition hover:bg-bgColor-brand/80 hover:text-slate-50 py-5"
      >
