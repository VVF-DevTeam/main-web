...
const EventListVertical = async ({
  ...
  return (
    <div className="flex w-full flex-col items-center gap-y-5 bg-[#FEFAF4] py-10 xl:py-14">
      {/* Header */}
      <h2 className="header-main text-center">
        {t(title || 'header-pastEvent')}
      </h2>
      ...
      {events.length > 0 ? (
        <div className="mx-auto max-w-7xl px-6 pt-5">
          ...
          <div className="hidden flex-col gap-6 md:flex lg:gap-8">
            {events.map((event) => (
              <EventCardHorizontal key={event.id} event={event} locale={locale} />
            ))}
          </div>
        </div>
      ) : (
      ...
      {fromHomePage && events.length > 0 && (
        <Button variant="default" className="mt-4 px-8 py-6 text-lg">
          <Link href="/events">{t('more-events')}</Link>
        </Button>
      )}
    </div>
  )
