...
const EventCard = ({ event }: EventCardProps) => {
  return (
    <div className="group relative flex w-[450px] cursor-pointer flex-col rounded-lg bg-slate-50 shadow-xl transition-all duration-300 ease-in-out hover:bg-slate-100 md:w-[350px] xl:w-[400px]">
      <Image
        src={event.imgUrl!}
        alt="event thumbnail"
        width={390}
      ...
      <div className="abosolute flex basis-1/2 flex-col gap-y-6 p-4 text-base md:text-lg">
        ...
        <div
          className={cn(
            event.eventType === 'CLASS'
              ...
              : 'rounded-2xl bg-yellow-900 px-2 py-2 text-center text-base font-semibold text-slate-50 opacity-100 transition-all duration-100 hover:bg-yellow-700/80'
          )}
        >
        ...
        <div className="ease flex items-center justify-between rounded-lg bg-[#C54B3E]/90 p-4 text-[#f7f1f1] transition-all duration-100 group-hover:bg-[#C54B3E]/90">
          ...
          <div className="flex flex-col text-right text-sm lg:text-lg">
            <span>
              {event.days.map((day) => <span key={day}>{day}</span>).join(', ')}
            </span>
            <span className="ml-auto">
              at{' '}
              {event.startTime.toLocaleTimeString('en-GB', {}).substring(0, 5)}
            </span>
          </div>
        </div>
      </div>
