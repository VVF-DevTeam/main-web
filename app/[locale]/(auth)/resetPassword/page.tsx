...
const ResetPasswordPage = () => {
  ...
  const resetPasswordSchema = z.object({
    password: z
      .string()
      .min(8, { message: t('password-requirements-description-1') || 'Password must be at least 8 characters long' })
      .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/, { message: t('password-requirements-description-2') || 'Password must contain at least one uppercase letter, one lowercase letter, and one number' }),
    confirmPassword: z
      .string()
      .min(8, { message: t('password-requirements-description-1') || 'Password must be at least 8 characters long' })
      .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/, { message: t('password-requirements-description-2') || 'Password must contain at least one uppercase letter, one lowercase letter, and one number' }),
